...
const sendmail = require('../service/student_mail')
...
module.exports.movie = function (req, res) {
  try {
    var fetch = new FetchStream(api_key);

    fetch.on("data", function (chunk) {
      var temp = JSON.parse(chunk.toString());
      console.log(temp);

      return responce.sendmessResponse(res, 'successfully', temp, status_code.STATUS_CODES.SUCCESS)
    });
  } catch (err) {
    console.log(err)
    return responce.sendResponse(res, 'There are some error', status_code.STATUS_CODES.UNAUTHORIZED)
  }
}
...
//   var fetch = new FetchStream("http://www.omdbapi.com/?i=tt3896198&apikey=7f62c3fe");
